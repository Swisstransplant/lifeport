...
This showcase imports 50 LifePort raw data sets and performs descriptive statistics. This report was created using R, RStudio, Quarto, and the Swisstransplant package (`swt`) among a few other packages. I hope this work will advance research and analysis of perfusion data.
...
Even the complete time series data can be accessed, below an example for the first kidney, the first five samples, and the first 
...
p4 = ggplot(data.timeseries[[k]][t,], aes(x = time.zero, y = FlowRate.flt)) + 
  ...
  xlab("Time starting at 0") + ylab("Flow rate (filtered)") +
